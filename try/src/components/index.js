...
import IDPhotoGroup from './IDPhotoGroup'
...
export const geneBox = (field, opts = {}) => {
  ...
  switch (field.type) {
    ...
    case 'IDPhotoGroup':
      return (
        <IDPhotoGroup
          ...
          data={field.data}
          form={opts.form}
          typeDisabled={field.typeDisabled}
          timeHidden={field.timeHidden}
          typeHidden={field.typeHidden}
          add={field.add}
          closeable={field.closeable}
    ...
    case 'number':
      let min = typeof field.min === 'number' ? field.min : undefined
      return (
        <InputNumber
          {...defaultOpts}
...
export const createFormItem = (opts) => {
  ...
  if (field.hidden && !field.search) {
    ...
    if (field.max && field.type !== 'number') {
      rules.push({ max: field.max, message: `${msgLabel}必须小于${field.max}个字符`,
        transform: (v) => {
          if (typeof v === 'number') {
            v += ''
          }
    ...
    if (field.min && field.type !== 'number') {
      rules.push({ min: field.min, message: `${msgLabel}必须大于${field.min}个字符`,
        transform: (v) => {
          if (typeof v === 'number') {
            v += ''
          }
